...
const JBSUpload: React.FC<JBSUploadProps> = ({
  sourceData,
  handleCallback,
  successCallback,
  sortIs = false,
  ...
  const uploadVideoImg = async (file: any, type: any) => {
    if (type) {
      try {
        ...
        const result = await getVideoImg(
          url,
          `${parseInt((Math.random() * 100000).toString())}.jpeg`,
        );
        const videoImgInfo = await tokenInfo?.put(
          `AIWO_INTELLECTUAL/image/${Date.now()}/${Date.now()}_${result?.name}`,
          result,
        );
  ...
  const multipartUpload = async (file, type) => {
    ...
    if (defaultName) {
      ...
      fileName = `${Date.now()}_${parseInt((Math.random() * 100000).toString())}.${
        arr[arr.length - 1]
      }`;
    }
    tokenInfo
      ...
      if (sortIs) {
        ...
        return (
          <DragableUploadListItem
            originNode={
              <div className="spaneT">
                <span>
                  <img
                    className="img"
                    src="https://aiwo-platform.oss-cn-hangzhou.aliyuncs.com/wechatgpt/chat/ic_excel_type.png"
                  />
                  <text>{file.name}</text>
                </span>
                <text
  ...
  const uploadBox =
    listType === 'text' ? (
      <Button className="uploadView">
        <div>
          <text className="text">点击上传</text>
          <text>或拖放文件置此处</text>
        </div>
        <text>（目前支持xlsx文件格式）</text>
      </Button>
    ) : fileListArr.length >= maxCount ? null : (
      uploadButton
    );
  ...
  return (
    <div className="jbsUpload">
      <DndProvider backend={HTML5Backend}>
        <Upload {...uploadProps}>{(disabled ? uploadState : false) ? null : uploadBox}</Upload>
        <Modal
          open={previewOpen}
          title={handleFileName(previewTitle)}
          footer={null}
